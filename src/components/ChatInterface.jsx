...
function ChatInterface({
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    try {
      ...
      return settings.selectedModel || "qwen-2.5-flash";
    } catch (e) {
      return "qwen-2.5-flash";
    }
  });
  ...
  useEffect(() => {
    const checkSettings = () => {
      try {
        ...
        setIsYoloMode(false);
        setSelectedModel("qwen-2.5-flash");
      }
    };
    ...
    (async () => {
      if (messages.length > 0) {
        ...
        switch (latestMessage.type) {
          ...
          case "qwen-response":
            const messageData =
              latestMessage.data.message || latestMessage.data;
            ...
            if (
              ...
              // Debug - qwen CLI session duplication detected
  ...
  const handleSubmit = async (e) => {
    ...
    const getToolsSettings = () => {
      try {
        ...
        if (savedSettings) {
          ...
          return {
            ...
            selectedModel: settings.selectedModel || "qwen-2.5-flash",
          };
        }
      } catch (error) {
      ...
      return {
        ...
        selectedModel: "qwen-2.5-flash",
      };
    };

    const toolsSettings = getToolsSettings();

    // Send command to qwen CLI via WebSocket with images
    sendMessage({
      type: "qwen-command",
      command: input,
      options: {
        ...
        model: toolsSettings.selectedModel || "qwen-2.5-flash",
      ...
      <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-gray-500 dark:text-gray-400 px-6 sm:px-4">
                <p className="font-bold text-lg sm:text-xl mb-3">
                  Start a conversation with qwen
                </p>
        ...
        >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full animate-pulse ${
                      isYoloMode ? "bg-orange-500" : "bg-cyan-500"
                    }`}
                  />
                  <span>{isYoloMode ? "qwen YOLO" : "qwen Default"}</span>
          ...
          <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
            ...
            <div
              {...getRootProps()}
              className={`chat-input-container relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 ${
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 text-center mt-2 hidden sm:block">
              Press Enter to send • Shift+Enter for new line • Tab to change
              modes • @ to reference files
            </div>
            <div
