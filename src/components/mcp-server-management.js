...
export default function McpServerManagement({ projects = [], setSaveStatus }) {
  ...
  const deleteMcpServer = async (serverId, scope = "user") => {
    try {
      const token = localStorage.getItem("auth-token");
      // Use qwen cli to remove the server with proper scope
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          throw new Error(
            result.error || "Failed to delete server via qwen cli"
          );
        }
      } else {
        const error = await response.json();
  ...
  const saveMcpServer = async (serverData) => {
    try {
      ...
      // Use qwen cli to add the server
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          throw new Error(result.error || "Failed to save server via qwen cli");
        }
      } else {
        const error = await response.json();
        ...
        children: [
          ...
          _jsx("div", {
            ...
            children: _jsx("p", {
              ...
              children:
                "Model Context Protocol servers provide additional tools and data sources to qwen",
            }),
          }),
          _jsx("div", {
