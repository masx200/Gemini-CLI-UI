...
router.get("/commit-diff", async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await execAsync(`git show ${commit}`, {
      cwd: projectPath,
    });
...
router.post("/generate-commit-message", async (req, res) => {
  ...
  try {
    ...
    // In a real implementation, you might want to use GPT or gemini API
...
router.post("/fetch", async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await execAsync(`git fetch ${remoteName}`, {
      cwd: projectPath,
    });
...
router.post("/discard", async (req, res) => {
  ...
  try {
    ...
    if (!statusOutput.trim()) {
      return res.status(400).json({
        error: "No changes to discard for this file",
      });
    }
