...
async function loadProjectConfig() {
    ...
    //@ts-ignore
    process.env.HOME || os.homedir(), ".gemini", "project-config.json");
    try {
...
async function extractProjectDirectory(projectName) {
    ...
    //@ts-ignore
    process.env.HOME || os.homedir(), ".gemini", "projects", projectName);
...
async function getProjects() {
    const geminiDir = path.join(//@ts-ignore
    process.env.HOME | os.homedir(), ".gemini", "projects");
        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    const paginatedSessions = allSessions.slice(0, 5); //@ts-ignore
                    project.sessions = paginatedSessions; //@ts-ignore
                    project.sessionMeta = {
    ...
    for (const [projectName, projectConfig] of Object.entries(config)) { //@ts-ignore
        if (!existingProjects.has(projectName) && projectConfig.manuallyAdded) {
            ...
            //@ts-ignore
            projects.push(project); //@ts-ignore
        }
    }
    return projects;
}
...
async function getSessionMessages(projectName, sessionId) {
    ...
    try {
        ...
        for (const file of jsonlFiles) {
            ...
            for await (const line of rl) {
                if (line.trim()) {
                    try {
                        const entry = JSON.parse(line);
                        if (entry.sessionId === sessionId) { //@ts-ignore 
                            //@ts-ignore
                            messages.push(entry);
                        }
                    }
        ...
        return messages.sort(//@ts-ignore
