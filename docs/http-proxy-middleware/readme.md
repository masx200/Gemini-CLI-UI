...
E.g. It's **not** guaranteed that `localhost` will be resolved to `127.0.0.1` 鈥� it might just as well be `::1` (or some other IP address).
...
> Note: There鈥檚 a thing called [Happy Eyeballs](https://en.wikipedia.org/wiki/Happy_Eyeballs) which means connecting to both IPv4 and IPv6 in parallel, which Node.js doesn鈥檛 have, but explains why for example `curl` can connect.
